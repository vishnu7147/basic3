...
fetch("https://api.openweathermap.org/data/2.5/forecast?q='+newName.value+'&appid=fb4535fd90f85d2ff5d18cb062885970")
.then(response => response.json())
.then(data => {

    //Getting the min and max values for each day
    for(i = 0; i<5; i++){
        document.getElementById("day" + (i+1) + "Min").innerHTML = "Min: " + Number(data.list[i].main.temp_min - 287.08).toFixed(1)+ "°";
        //Number(1.3450001).toFixed(2); // 1.35
    }

    for(i = 0; i<5; i++){
        document.getElementById("day" + (i+1) + "Max").innerHTML = "Max: " + Number(data.list[i].main.temp_max - 288.97).toFixed(2) + "°";
    }
    //------------------------------------------------------------
